...
object NetModule {
    private val WEBRTC_BASE_URL = "https://apprtc.webrtcserver.cn/"
...
//                    urlBuilder.addEncodedQueryParameter("key", JNIHelper.getPrivateKey())
    ...
    @Provides
    @Named("addressUrl")
    fun provideWebrtcRetrofit(
        baseRetrofit: Retrofit.Builder
    ): Retrofit {
        return baseRetrofit.baseUrl(WEBRTC_BASE_URL).build()
    }
...
//        retrofit.create(WeatherInfoService::class.java)
    ...
    @Singleton
    @Provides
    fun providerWebrtcService(@Named("addressUrl") retrofit: Retrofit): WebrtcService =
        retrofit.create(WebrtcService::class.java)
