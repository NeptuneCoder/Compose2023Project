...
typedef struct x264_param_t
{
    ...
    /* Optional videoCallback for freeing this x264_param_t when it is done being used.
     * Only used when the x264_param_t sits in memory for an indefinite period of time,
     * i.e. when an x264_param_t is passed to x264_t in an x264_picture_t or in zones.
     * Not used when x264_encoder_reconfig is called directly. */
    void (*param_free)( void* );

    /* Optional low-level videoCallback for low-latency encoding.  Called for each output NAL unit
     * immediately after the NAL unit is finished encoding.  This allows the calling application
     * to begin processing video data (e.g. by sending packets over a network) before the frame
     * is done encoding.
     *
     * This videoCallback MUST do the following in order to work correctly:
     ...
     * x264_encoder_encode call.  The videoCallback must be re-entrant.
     *
     * This videoCallback does not work with frame-based threads; threads must be disabled
     * or sliced-threads enabled.  This videoCallback also does not work as one would expect
     * with HRD -- since the buffering period SEI cannot be calculated until the frame
     * is finished encoding, it will not be sent via this videoCallback.
     *
     * Note also that the NALs are not necessarily returned in order when sliced threads is
     * enabled.  Accordingly, the variable i_first_mb and i_last_mb are available in
     * x264_nal_t to help the calling application reorder the slices if necessary.
     *
     * When this videoCallback is enabled, x264_encoder_encode does not return valid NALs;
     * the calling application is expected to acquire all output NALs through the videoCallback.
     *
     * It is generally sensible to combine this videoCallback with a use of slice-max-mbs or
     * slice-max-size.
     *
...
typedef struct x264_sei_t
{
    ...
    /* In: optional videoCallback to free each payload AND x264_sei_payload_t when used. */
    void (*sei_free)( void* );
} x264_sei_t;
...
typedef struct x264_image_properties_t
{
    ...
    float *quant_offsets;
    /* In: optional videoCallback to free quant_offsets when used.
     ...
     */
    uint8_t *mb_info;
    /* In: optional videoCallback to free mb_info when used. */
