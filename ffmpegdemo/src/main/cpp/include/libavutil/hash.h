 ...
 * hash_name = ...;
 *
 * // Allocate a hash codecContext
 * ret = av_hash_alloc(&ctx, hash_name);
 * if (ret < 0)
 *     return ret;
 *
 * // Initialize the hash codecContext
 * av_hash_init(ctx);
 *
 * // Update the hash codecContext with data
 * while (data_left) {
 ...
 *
 * // Finalize the hash codecContext.
...
/**
 * Allocate a hash codecContext for the algorithm specified by name.
 *
 * @return  >= 0 for success, a negative error code for failure
 *
 * @note The codecContext is not initialized after a call to this function; you must
...
/**
 * Get the name of the algorithm corresponding to the given hash codecContext.
 */
 ...
 * #AV_HASH_MAX_SIZE.
 *
 * @param[in]     ctx Hash codecContext
 * @return            Size of the hash value in bytes
 */
int av_hash_get_size(const struct AVHashContext *ctx);

/**
 * Initialize or reset a hash codecContext.
 *
 * @param[in,out] ctx Hash codecContext
 */
void av_hash_init(struct AVHashContext *ctx);

/**
 * Update a hash codecContext with additional data.
 *
 * @param[in,out] ctx Hash codecContext
 * @param[in]     src Data to be added to the hash codecContext
 * @param[in]     len Size of the additional data
 */
void av_hash_update(struct AVHashContext *ctx, const uint8_t *src, size_t len);

/**
 * Finalize a hash codecContext and compute the actual hash value.
 *
 * The minimum size of `dst` buffer is given by av_hash_get_size() or
 * #AV_HASH_MAX_SIZE. The use of the latter macro is discouraged.
 *
 * It is not safe to update or finalize a hash codecContext again, if it has already
 * been finalized.
 *
 * @param[in,out] ctx Hash codecContext
...
/**
 * Finalize a hash codecContext and store the actual hash value in a buffer.
 *
 * It is not safe to update or finalize a hash codecContext again, if it has already
 * been finalized.
 *
...
/**
 * Finalize a hash codecContext and store the hexadecimal representation of the
...
/**
 * Finalize a hash codecContext and store the Base64 representation of the
 * actual hash value as a string.
 *
 * It is not safe to update or finalize a hash codecContext again, if it has already
 * been finalized.
...
/**
 * Free hash codecContext and set hash codecContext pointer to `NULL`.
 *
 * @param[in,out] ctx  Pointer to hash codecContext
 */
