...
typedef struct AVOption {
    ...
    /**
     * The offset relative to the codecContext structure where the option
 ...
 * options for which it is !(opt->flags & req_flags).
 * @param av_log_obj log codecContext to use for showing the options
 */
 ...
 * key. ctx must be an AVClass codecContext, storing is done using
 * AVOptions.
 *
 ...
 *
 * This list is constructed without using the AVClass.query_ranges() videoCallback
 * and can be used as fallback from within the videoCallback.
 *
 * @param flags is a bitmask of flags, undefined flags should not be set and should be ignored
 *              AV_OPT_SEARCH_FAKE_OBJ indicates that the obj is a double pointer to a AVClass instead of a full instance
 *              AV_OPT_MULTI_COMPONENT_RANGE indicates that function may return more than one component, @see AVOptionRanges
