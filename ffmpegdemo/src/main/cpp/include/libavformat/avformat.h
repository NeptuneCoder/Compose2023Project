 ...
 * @li an @ref AVFormatContext.pb "I/O codecContext". It is either opened by lavf or
 ...
 * Further options may be provided by the @ref AVFormatContext.pb "I/O codecContext",
 ...
 * options on a preallocated codecContext. Instead, the options should be passed to
 ...
 * avformat_alloc_context() to create a muxing codecContext. The caller then sets up
 * the muxer by filling the various fields in this codecContext:
 *
 * - The @ref AVFormatContext.oformat "oformat" field must be set to select the
 *   muxer that will be used.
 * - Unless the format is of the AVFMT_NOFILE type, the @ref AVFormatContext.pb
 *   "pb" field must be set to an opened IO codecContext, either returned from
 ...
 *   remuxing: there is no guarantee that the codec codecContext values remain valid
 ...
 *   support.
 *
 * When the muxing codecContext is fully set up, the caller must call
 * avformat_write_header() to initialize the muxer internals and write the file
 * header. Whether anything actually is written to the IO codecContext at this step
 ...
 * a single muxing codecContext, they should not be mixed). Do note that the timing
 ...
 * codecContext (if any) and finally free the muxing codecContext with
 ...
 *
 * @param s    associated IO codecContext
 * @param pkt packet
...
typedef struct AVOutputFormat {
    ...
    const AVClass *priv_class; ///< AVClass for the private codecContext
} AVOutputFormat;
/**
 * @}
 */
...
typedef struct AVFormatContext {
     ...
     *           take care of closing / freeing the IO codecContext.
     *
...
#define AVFMT_AVOID_NEG_TS_MAKE_ZERO         2 ///< Shift timestamps so that they start at 0
    ...
    /**
     * Flush the I/O codecContext after each packet.
     * - encoding: Set by user
     * - decoding: unused
     */
    int flush_packets;
     ...
     * This is set by avformat when the underlaying IO codecContext read pointer
     ...
     *
     * @param s the format codecContext
     * @param pb IO codecContext to be closed and freed
     * @return 0 on success, a negative AVERROR code on failure
     */
 ...
 * avformat_free_context() can be used to free the codecContext and everything
 ...
 * avformat_free_context() can be used to free the codecContext and
 * everything allocated by the framework within it.
 *
 * @param ctx           pointee is set to the created format codecContext,
 *                      or to NULL in case of failure
 * @param oformat       format to use for allocating the codecContext, if NULL
 *                      format_name and filename are used instead
 * @param format_name   the name of output format to use for allocating the
 *                      codecContext, if NULL filename is used instead
 * @param filename      the name of the filename to use for allocating the
 *                      codecContext, may be NULL
 *
 ...
 * @param logctx         the log codecContext
 ...
 *
 * @note If you want to use custom IO, preallocate the format codecContext and set its pb field.
 */
 ...
 * codec and time base.
 *
 * @param ic        the codecContext to analyze
 * @param index     index of the stream to dump information about
 * @param url       the URL to print, such as source or destination file
 * @param is_output Select whether the specified codecContext is an input(0) or output(1)
 */
 ...
 *           array is composed by only one codecContext, such codecContext can contain
 ...
 *
 * @param format the format codecContext which the stream is part of
 ...
 *
 * @param ctx the format codecContext which the stream is part of
 * @param stream the stream which the frame is part of
