...
struct AVFilterContext {
     ...
     * hardware codecContext information.
     */
 ...
 *
 * @param ctx  uninitialized filter codecContext to initialize
...
/**
 * Free a filter codecContext. This will also remove the filter from its
 ...
 *
 * @param ctx the filter codecContext to which the jobs belong
 ...
 *
 * @return the codecContext of the newly created filter instance (note that it is
 ...
 * @param log_ctx codecContext used for logging
 ...
 * filter codecContext and the pad index required for establishing a link.
 */
typedef struct AVFilterInOut {
    /** unique name for this input/output in the list */
    char *name;

    /** filter codecContext associated to this input/output */
 ...
 *
 * @param graph   the filter graph where to link the parsed graph codecContext
 ...
 * Add a graph described by a string to a graph.
 *
 * @param[in]  graph   the filter graph where to link the parsed graph codecContext
 ...
 * AVFilterParams.filter is set to the newly-created filter codecContext,
 ...
 * no associated filter codecContext are simply skipped.
 *
 * @param seg the filtergraph segment to process
 * @param flags reserved for future use, caller must set to 0 for now
 *
 ...
 * Any creation-pending filters (see avfilter_graph_segment_create_filters())
 * present in the segment will cause this function to fail. AVFilterParams with
 * no associated filter codecContext or whose filter codecContext is already initialized,
 * are simply skipped.
 *
 * @param seg the filtergraph segment to process
 * @param flags reserved for future use, caller must set to 0 for now
